...
const db = require("../models"); // accès  aux tables de MySQL
const fs = require("fs"); //
...
// afficher les posts les plus likés par ordre croissant 
exports.getLikePosts = async (req, res) => {
  try {
    ...
    return res.status(500).send({
      error: "Une erreur est survenu lors de la récupération des posts filtrés par les likes ",
    });
  }
};
exports.getOnePost = async (req, res) => {
  try {
    const post = await db.Post.findOne({
      // on récupère le post avec l'id inclu et les tables , attributs des différents models (User, Like, Comment)
...
exports.createPost = async (req, res) => {
  ...
  try {
    ...
    if (user !== null) {
      ...
      res
        .status(201)
        .json({ post: post, messageRetour: "Votre post est ajouté avec succés" });
    } else {
      res.status(400).send({ error: "Erreur lors de l'ajout de votre post " });
    }
  } catch (error) {
    return res.status(500).send({ error: "Erreur serveur veuillez réessayer " });
  }
};
...
exports.updatePost = async (req, res) => {
  try {
    ...
    if (userId === post.UserId) {
      ...
      res.status(200).json({ newPost: newPost, messageRetour: "post modifié avec succés" });
    } else {
      res.status(400).json({ message: "Vous n'avez pas les droits requis pour modifier" });
    }
  } catch (error) {
    return res.status(500).send({ error: "Erreur serveur" });
  }
...
exports.likePost = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      res.status(200).send({ messageRetour: "Vous avez enlevé le like sur ce post" });
    } else {
...
exports.addComment = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ newComment, messageRetour: "votre commentaire est publié avec succés" });
  } catch (error) {
    return res.status(500).send({ error: "Erreur serveur" });
  }
};
