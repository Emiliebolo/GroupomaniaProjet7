const bcrypt = require("bcryptjs"); // Sécurité du password
const db = require("../models"); //  tous les modèles de la base de donnée
const token = require("../middleware/token"); //  Génère le token
const fs = require("fs");
  ...
  try {
    ...
    if (user !== null) {
      if (user.pseudo === req.body.pseudo) {
        return res.status(400).json({ error: "ce pseudo est déjà utilisé, veuillez essayer un autre " });
      }
      console.log(user);
    } else {
...
exports.login = async (req, res) => {
  try {
    const user = await db.User.findOne({
      where: { email: req.body.email },
    }); //Permet de vérifier si l'adresse email existe dans la base de donnée
    if (user === null) {
      return res.status(403).send({ error: "l'adresse email n'existe pas" });
    } else {
      ...
      if (!hash) {
        ...
        res.status(200).send({
          ...
          sub: tokenObject.sub,
          expires: tokenObject.expiresIn,
          message: "De retour parmis nous " + user.pseudo + " !",
        });
      }
    }
  } catch (error) {
...
exports.getAccount = async (req, res) => {
  // on trouve l'utilisateur dans la base de donnée et on renvoie l'objet user
  try {
    const user = await db.User.findOne({
...
exports.updateAccount = async (req, res) => {
  // modifier le profil en utilisant le token
  ...
  try {
    ...
    if (user.photo !== null) {
      ...
      fs.unlink(`upload/${filename}`, () => {
        // si il' y a une photo on la supprime et on supprime le compte
        db.User.destroy({ where: { id: id } });
        res.status(200).json({ messageRetour: "utilisateur supprimé" });
      });
    } else {
      db.User.destroy({ where: { id: id } }); // on supprime le compte sans photo 
      res.status(200).json({ messageRetour: "utilisateur supprimé" });
    }
  } catch (error) {
    return res.status(500).send({ error: "Erreur serveur" });
